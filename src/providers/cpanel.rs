...
impl DnsProvider for Cpanel {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let hostname = env.get("CPANEL_Hostname")
            .ok_or_else(|| Error::Config("CPANEL_Hostname required".into()))?.clone();
        let username = env.get("CPANEL_Username")
            .ok_or_else(|| Error::Config("CPANEL_Username required".into()))?.clone();
        let api_token = env.get("CPANEL_ApiToken")
            .ok_or_else(|| Error::Config("CPANEL_ApiToken required".into()))?.clone();
        Ok(Box::new(Cpanel { hostname, username, api_token }))
    }
    ...
    fn add_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        let resp = http::get(&url, &[("Authorization", &auth)])
            .map_err(|e| Error::Provider(format!("cpanel add TXT: {e}")))?;
        let v: Value = serde_json::from_str(&resp.body)
            .map_err(|e| Error::Json(format!("cpanel response: {e}")))?;
            ...
            if let Some(first) = data.first() {
                if let Some(result) = first.get("result") {
                    ...
                    if let Some(msg) = result.get("statusmsg").and_then(|m| m.as_str()) {
                        return Err(Error::Provider(format!("cpanel add TXT: {msg}")));
                    }
                }
            }
        }
        if let Some(err) = v.get("cpanelresult").and_then(|r| r.get("error")).and_then(|e| e.as_str()) {
            return Err(Error::Provider(format!("cpanel add TXT: {err}")));
        }
        Err(Error::Provider(format!("cpanel add TXT: unexpected response: {}", resp.body)))
    }
    ...
    fn remove_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        match self.find_record_line(domain, name, value) {
            ...
            Ok(None) => Ok(()),
            Err(e) => {
                eprintln!("warning: cpanel cleanup failed: {e}");
                Ok(())
            }
        }
    }
...
impl Cpanel {
    fn find_record_line(&self, domain: &str, name: &str, value: &str) -> Result<Option<String>, Error> {
        ...
        let auth = format!("cpanel {}:{}", self.username, self.api_token);
        let resp = http::get(&url, &[("Authorization", &auth)])
            .map_err(|e| Error::Provider(format!("cpanel fetchzone: {e}")))?;
        let v: Value = serde_json::from_str(&resp.body)
            .map_err(|e| Error::Json(format!("cpanel fetchzone: {e}")))?;
        if let Some(data) = v.get("cpanelresult").and_then(|r| r.get("data")).and_then(|d| d.as_array()) {
            for record in data {
