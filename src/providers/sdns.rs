...
const BASE_URL: &str = "https://api.s-dns.de";
...
impl DnsProvider for Sdns {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let username = env.get("SDNS_Username")
            .ok_or_else(|| Error::Config("SDNS_Username required".into()))?
            .clone();
        let password = env.get("SDNS_Password")
            .ok_or_else(|| Error::Config("SDNS_Password required".into()))?
            .clone();
        Ok(Box::new(Sdns { username, password }))
    }
    ...
    fn add_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        let resp = http::post(&url, &serde_json::to_vec(&body).unwrap(), "application/json",
            &[("Authorization", &auth)])
            .map_err(|e| Error::Provider(format!("sdns add TXT: {e}")))?;
        if resp.status >= 400 {
            return Err(Error::Provider(format!("sdns add TXT: HTTP {} {}", resp.status, resp.body)));
        }
        Ok(())
    }
