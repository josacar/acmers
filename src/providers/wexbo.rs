...
const BASE_URL: &str = "https://api.wexbo.de/v1";
...
impl DnsProvider for Wexbo {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let username = env.get("WEXBO_User")
            .ok_or_else(|| Error::Config("WEXBO_User required".into()))?
            .clone();
        let password = env.get("WEXBO_Password")
            .ok_or_else(|| Error::Config("WEXBO_Password required".into()))?
            .clone();
        Ok(Box::new(Wexbo { username, password }))
    }
    ...
    fn add_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        let resp = http::post(&url, &serde_json::to_vec(&body).unwrap(), "application/json",
            &[("Authorization", &auth)])
            .map_err(|e| Error::Provider(format!("wexbo add TXT: {e}")))?;
        if resp.status >= 400 {
            return Err(Error::Provider(format!("wexbo add TXT: HTTP {} {}", resp.status, resp.body)));
        }
        Ok(())
    }
