...
const BASE_URL: &str = "https://api.spaceship.io/v1";
...
impl DnsProvider for Spaceship {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let key = env.get("SPACESHIP_Key")
            .ok_or_else(|| Error::Config("SPACESHIP_Key required".into()))?
            .clone();
        let secret = env.get("SPACESHIP_Secret")
            .ok_or_else(|| Error::Config("SPACESHIP_Secret required".into()))?
            .clone();
        Ok(Box::new(Spaceship { key, secret }))
    }
