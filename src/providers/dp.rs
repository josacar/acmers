...
const BASE_URL: &str = "https://dnsapi.cn";
...
impl DnsProvider for Dnspod {
    ...
    fn add_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        let resp = http::post(&format!("{BASE_URL}/Record.Create"), body.as_bytes(), "application/x-www-form-urlencoded", &[])
            .map_err(|e| Error::Provider(format!("DNSPod add TXT: {e}")))?;

        if resp.status >= 300 {
            return Err(Error::Provider(format!("DNSPod add TXT: {} {}", resp.status, resp.body)));
        }

        let v: serde_json::Value = serde_json::from_str(&resp.body)
            .map_err(|e| Error::Json(format!("DNSPod response: {e}")))?;
        if let Some(err_msg) = v.get("status").and_then(|s| s.get("message")).and_then(|m| m.as_str()) {
            ...
            if code != "1" {
                return Err(Error::Provider(format!("DNSPod add TXT: {err_msg}")));
            }
        }
        Ok(())
    }
    ...
    fn remove_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        // DNSPod uses Record.Remove endpoint
...
impl Dnspod {
    fn resolve_domain(&self, domain: &str) -> Result<String, Error> {
        ...
        let resp = http::post(&format!("{BASE_URL}/Domain.List"), body.as_bytes(), "application/x-www-form-urlencoded", &[])
            .map_err(|e| Error::Provider(format!("DNSPod list domains: {e}")))?;

        if resp.status >= 300 {
            return Err(Error::Provider(format!("DNSPod list domains: {} {}", resp.status, resp.body)));
        }

        let v: serde_json::Value = serde_json::from_str(&resp.body)
            .map_err(|e| Error::Json(format!("DNSPod domains: {e}")))?;
    ...
    fn find_record(&self, domain_id: &str, short_name: &str, value: &str) -> Result<Option<String>, Error> {
        ...
        let resp = http::post(&list_url, body.as_bytes(), "application/x-www-form-urlencoded", &[])
            .map_err(|e| Error::Provider(format!("DNSPod list records: {e}")))?;
        ...
        let v: serde_json::Value = serde_json::from_str(&resp.body)
            .map_err(|e| Error::Json(format!("DNSPod records: {e}")))?;
