...
const BASE_URL: &str = "https://rest.selfhost.de/api/v1";
...
impl DnsProvider for Selfhost {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let username = env.get("SELFHOST_Username")
            .ok_or_else(|| Error::Config("SELFHOST_Username required".into()))?
            .clone();
        let password = env.get("SELFHOST_Password")
            .ok_or_else(|| Error::Config("SELFHOST_Password required".into()))?
            .clone();
        Ok(Box::new(Selfhost { username, password }))
    }
    ...
    fn add_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        let url = format!("{BASE_URL}/domain/{domain}/records");
    ...
    fn remove_txt(&self, domain: &str, name: &str, _value: &str) -> ProviderResult {
        ...
        let list_url = format!("{BASE_URL}/domain/{domain}/records");
            ...
            for record in records {
                    ...
                    && record.get("name").and_then(|n| n.as_str()) == Some(rec_name)
                {
                    if let Some(id) = record_id(record) {
                        let del_url = format!("{BASE_URL}/domain/{domain}/records/{id}");
