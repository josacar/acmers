...
const BASE_URL: &str = "https://api.selectel.ru/dns/v1";
...
impl DnsProvider for Selectel {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let username = env.get("SELECTEL_Username")
            .ok_or_else(|| Error::Config("SELECTEL_Username required".into()))?
            .clone();
        let password = env.get("SELECTEL_Password")
            .ok_or_else(|| Error::Config("SELECTEL_Password required".into()))?
            .clone();
        Ok(Box::new(Selectel { username, password }))
    }
    ...
    fn add_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        let resp = http::post(&url, &serde_json::to_vec(&body).unwrap(), "application/json",
            &[("Authorization", &auth)])
            .map_err(|e| Error::Provider(format!("selectel add TXT: {e}")))?;
        if resp.status >= 400 {
            return Err(Error::Provider(format!("selectel add TXT: HTTP {} {}", resp.status, resp.body)));
        }
        Ok(())
    }
