...
const BASE_URL: &str = "https://api.auroradns.eu/api/v1";
...
impl DnsProvider for Aurora {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let key = env.get("AURORA_Key")
            .ok_or_else(|| Error::Config("AURORA_Key required".into()))?.clone();
        let secret = env.get("AURORA_Secret")
    ...
    fn add_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        if resp.status >= 400 {
            return Err(Error::Provider(format!("aurora add TXT: HTTP {} {}", resp.status, resp.body)));
        }
        Ok(())
    }
