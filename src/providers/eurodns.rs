...
const BASE_URL: &str = "https://api.eurodns.com/api/v1";

pub struct Eurodns {
    basic_auth: String,
}

impl DnsProvider for Eurodns {
    fn slug() -> &'static str { "eurodns" }
    fn env_vars() -> &'static [&'static str] { &["EURODNS_ID", "EURODNS_KEY"] }

    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let id = env.get("EURODNS_ID")
            .ok_or_else(|| Error::Config("EURODNS_ID required".into()))?.clone();
        let key = env.get("EURODNS_KEY")
            .ok_or_else(|| Error::Config("EURODNS_KEY required".into()))?.clone();
        let creds = format!("{id}:{key}");
        let basic_auth = format!("Basic {}", base64::encode_std(creds.as_bytes()));
        Ok(Box::new(Eurodns { basic_auth }))
    }
    ...
    fn add_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        let resp = http::post(&url, &serde_json::to_vec(&body).unwrap(), "application/json", headers)
            .map_err(|e| Error::Provider(format!("eurodns add TXT: {e}")))?;
        if resp.status >= 400 {
            return Err(Error::Provider(format!("eurodns add TXT: HTTP {} {}", resp.status, resp.body)));
        }
        Ok(())
    }
