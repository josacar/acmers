...
const BASE_URL: &str = "https://api.kinghost.net/v1";
...
impl DnsProvider for Kinghost {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let api_key = env.get("KINGHOST_ApiKey")
            .ok_or_else(|| Error::Config("KINGHOST_ApiKey required".into()))?
            .clone();
        let secret = env.get("KINGHOST_Secret")
            .ok_or_else(|| Error::Config("KINGHOST_Secret required".into()))?
            .clone();
        Ok(Box::new(Kinghost { api_key, secret }))
    }
    ...
    fn add_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        let resp = http::post(&url, &serde_json::to_vec(&body).unwrap(), "application/json",
            &[("X-API-Key", &self.api_key), ("X-API-Secret", &self.secret)])
            .map_err(|e| Error::Provider(format!("kinghost add TXT: {e}")))?;
        if resp.status >= 400 {
            return Err(Error::Provider(format!("kinghost add TXT: HTTP {} {}", resp.status, resp.body)));
        }
        Ok(())
    }
    ...
    fn remove_txt(&self, domain: &str, name: &str, _value: &str) -> ProviderResult {
        ...
        let headers: &[(&str, &str)] = &[("X-API-Key", &self.api_key), ("X-API-Secret", &self.secret)];
...
impl Kinghost {
    fn resolve_zone(&self, domain: &str) -> Result<String, Error> {
        ...
        let resp = http::get(&format!("{BASE_URL}/dns/zones"), headers)
            .or_else(|_| http::get(&format!("{BASE_URL}/domains"), headers))
            .map_err(|e| Error::Provider(format!("kinghost list zones: {e}")))?;
        let v: Value = serde_json::from_str(&resp.body)
            .map_err(|e| Error::Json(format!("kinghost zones: {e}")))?;
