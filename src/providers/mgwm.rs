...
const BASE_URL: &str = "https://api.mgw-media.de";
...
impl DnsProvider for Mgwm {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let username = env.get("MGWM_USERNAME")
            .ok_or_else(|| Error::Config("MGWM_USERNAME required".into()))?
            .clone();
        let password = env.get("MGWM_PASSWORD")
            .ok_or_else(|| Error::Config("MGWM_PASSWORD required".into()))?
            .clone();
        Ok(Box::new(Mgwm { username, password }))
    }
