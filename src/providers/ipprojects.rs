...
const BASE_URL: &str = "https://api.ip-projects.de/api/v1";
...
impl DnsProvider for Ipprojects {
    ...
    fn env_vars() -> &'static [&'static str] { &["IPPROJECTS_Key", "IPPROJECTS_Secret"] }

    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let key = env.get("IPPROJECTS_Key")
            .ok_or_else(|| Error::Config("IPPROJECTS_Key required".into()))?.clone();
        let secret = env.get("IPPROJECTS_Secret")
            .ok_or_else(|| Error::Config("IPPROJECTS_Secret required".into()))?.clone();
        Ok(Box::new(Ipprojects { key, secret }))
    }
