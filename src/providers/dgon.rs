...
impl DnsProvider for Digitalocean {
    ...
    fn remove_txt(&self, domain: &str, name: &str, value: &str) -> ProviderResult {
        ...
        let mut list_url = format!("https://api.digitalocean.com/v2/domains/{zone_name}/records?type=TXT");
        loop {
            ...
            match next_page_url(&v) {
                Some(next) => list_url = next,
                None => break,
            }
        }
        Ok(())
...
fn next_page_url(v: &Value) -> Option<String> {
    v.get("links")?
        .get("pages")?
        .get("next")?
        .as_str()
