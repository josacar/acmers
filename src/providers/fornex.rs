...
const BASE_URL: &str = "https://api.fornex.com/v1";
...
impl DnsProvider for Fornex {
    ...
    fn new(env: &HashMap<String, String>) -> Result<Box<dyn DnsProvider>, Error> {
        let username = env.get("FORNEX_Username")
            .ok_or_else(|| Error::Config("FORNEX_Username required".into()))?
            .clone();
        let password = env.get("FORNEX_Password")
            .ok_or_else(|| Error::Config("FORNEX_Password required".into()))?
            .clone();
        let creds = base64::encode_std(format!("{username}:{password}").as_bytes());
        let auth = format!("Basic {creds}");
        Ok(Box::new(Fornex { auth }))
    }
...
impl Fornex {
    fn resolve_zone(&self, domain: &str) -> Result<String, Error> {
        let resp = http::get(&format!("{BASE_URL}/dns/zones"), &[("Authorization", &self.auth)])
            ...
            .map_err(|e| Error::Provider(format!("fornex list zones: {e}")))?;
        let v: Value = serde_json::from_str(&resp.body)
            .map_err(|e| Error::Json(format!("fornex zones: {e}")))?;
