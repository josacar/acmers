...
The test HTTP base URL override SHALL be stored in thread-local storage so that
concurrent test threads do not interfere with each other's mock server URLs.
...
The URL rewrite logic SHALL extract the path from the original URL (scheme + host + path)
and append it to the thread-local base URL exactly as before.
