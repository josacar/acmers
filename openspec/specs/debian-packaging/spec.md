...
Generates installable `.deb` packages for the acmers binary, supporting amd64 and arm64 architectures. Uses a manual `dpkg-deb` approach (no `cargo-deb` dependency) to keep the build toolchain minimal.
